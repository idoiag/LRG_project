...
def get_up_anno(data):
    ...
    for annotation in root.findall('./updatable_annotation/annotation_set')[2] :
        for mapping in root.findall('./mapping/mapping_span'):
            str_dir = mapping.get('strand')
            print (str_dir)
            print ("Hello")
...
def exon_lst(data):
    exon_lst = tree.findall('fixed_annotation/transcript/exon')
    print('Exon count: ', len(exon_lst))
    return

     #### NEEDS CHECKING ###
"""
lrg_id2= tree.find('fixed_annotation/id').text
print (lrg_id2)
...
#print('Exon number: ', exons.get("label"))
    ...
    if int(exon_number) > 0:
        for coordinates in exons:
            if coordinates.get('coord_system') == lrg_id2:
...
exon_lst(data)
#(exon_number, coord_start, coord_end, strand) = exon_lst(data, lrg_id)
